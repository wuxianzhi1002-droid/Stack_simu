...
def generate_lsf(output_filename="run_stack_simulation.lsf"):
    ...
    lsf_content.append(f"f = {list(mm.freqs)}; # Frequency vector")
    lsf_content.append("")
    ...
    for stack in stacks:
        ...
        lsf_content.append(f"d_{name} = {thicknesses};")
        ...
        for i, (mat_name, _) in enumerate(layers):
            if "custom" in mat_name:
                # For custom materials, we'll need to manually define the index in LSF or assume they are added
                n, k = getattr(mm, f"get_{mat_name.split('_')[0].lower()}")()
                n_complex = n + 1j*k
                # Convert complex array to string for LSF (real + 1i*imag)
                n_str = "{" + ",".join([f"{val.real}+{val.imag}i" for val in n_complex]) + "}"

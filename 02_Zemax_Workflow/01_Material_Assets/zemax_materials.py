...
class ZemaxMaterialManager:
    ...
    def __init__(self, catalog_name="STACK_MATERIALS"):
        ...
        self.materials = {
            ...
            "SI3N4_custom": {"A": 1.97, "B": 0.01}, # 典型的氮化硅色散模型
...
if __name__ == "__main__":
    ...
    # 将 Lumerical Workflow 目录添加到路径，以便导入 stack_builder 和 materials
    base_dir = Path(__file__).parent.parent.parent
    sys.path.append(str(base_dir / "01_Lumerical_Workflow"))
    ...
    # 动态获取当前脚本所在目录，确保文件保存在正确位置
    script_dir = os.path.dirname(os.path.abspath(__file__))
    zmm.generate_agf(os.path.join(script_dir, "STACK_MATERIALS.AGF"))
    zmm.generate_coating_dat(os.path.join(script_dir, "COATING.DAT"), stacks)

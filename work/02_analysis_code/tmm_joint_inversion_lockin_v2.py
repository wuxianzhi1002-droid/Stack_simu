...
def tmm_reflectance(
    ...
    if not np.isclose(theta_rad, 0.0):
        raise ValueError("The StackRT-matched v2 TMM currently supports normal incidence only.")
...
def fit_mode(measurement: dict, config: FitConfig, mode: str, rng: np.random.Generator) -> dict:
    ...
    # A failed attempt must never displace a converged solution solely because
    # it stopped at a slightly lower intermediate cost.
...
def run(config: FitConfig, modes: list[str]) -> Path:
    ...
    output_dir = OUTPUT_ROOT / f"tmm_joint_inversion_lockin_v2_{time.strftime('%Y%m%d_%H%M%S')}"
        ...
        json.dumps(
            {
                ...
                "tmm_convention": {
                    ...
                    "multistart_selection": "successful attempts first, then ascending cost",
                },

...
def stackrt_material_n(
    ...
    if name == "Cu":
        if fdtd is not None and freqs is not None:
            try:
                return np.asarray(fdtd.getindex("Cu (Copper) - Palik", freqs)).flatten().astype(np.complex128)
            except Exception:
                pass
...
def run_validation(
    ...
    output_dir = OUTPUT_ROOT / f"stackrt_vs_tmm_inverse_validation_{time.strftime('%Y%m%d_%H%M%S')}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare TMM inverse fitting on TMM-generated and StackRT-generated spectra.")
    ...
    parser.add_argument("--truth-samples", type=int, default=50)

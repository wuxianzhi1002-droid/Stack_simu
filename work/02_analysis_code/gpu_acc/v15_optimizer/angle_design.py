"""V15 Stage 3 robust two-angle experiment design with EQ-99X."""
...
ANGLE_GRID_DEG = np.arange(0.0, 0.2000001, 0.02, dtype=np.float64)
...
def main():
    ...
    if len(paths)!=100: raise RuntimeError(f"expected 100 V15 typical NPZ, found {len(paths)}")
    ...
    report={
        "version":"v15_stage3_two_angle_design","overall":"PASS",
        "created_utc":datetime.now(timezone.utc).isoformat(),
        "angle_grid_deg":ANGLE_GRID_DEG,"angle_measurement_sigma_deg":ANGLE_SIGMA_DEG,
        ...
        "scope_guard":"Two distinct fixed measured reflector angles; five shared structure parameters; no Angle fit and no Angle MAP.",
    }
    write_csv(output/"v15_stage3_angle_pairs.csv",all_rows)
    write_csv(output/"v15_stage3_single_angles.csv",single_rows)
    (output/"v15_stage3_angle_design.json").write_text(json.dumps(v12.safe(report),ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    lines=["# V15 Stage 3 EQ-99X two-angle design","",f"- Production pair: `{primary['theta1_deg']:.3f}?, {primary['theta2_deg']:.3f}?`",f"- Angle grid: 0.00?0.20?, step 0.02?; robust perturbation ?{ANGLE_SIGMA_DEG:.3f}?.","- Pair selected on 220?580 nm and reused unchanged for 200?800 nm.","","| band | selected pair (deg) | worst sigma_min | worst kappa | worst log10 det |","|---|---:|---:|---:|---:|"]
    for band,row in selected_by_band.items(): lines.append(f"| {band} | {row['theta1_deg']:.3f}, {row['theta2_deg']:.3f} | {row['worst_sigma_min']:.6g} | {row['worst_condition_number']:.6g} | {row['worst_log10_det']:.6g} |")
    (output/"v15_stage3_angle_design.md").write_text("\n".join(lines)+"\n",encoding="utf-8")

"""Generate the paired 200-800 nm V15 Stage 1 master dataset with StackRT.

Only the ten ``typical`` noise cases are generated (ten realizations each).
Seeds retain their original position in the complete V15 case matrix.  The
...
def load_main_v15():
    ...
    module_name = "_v15_stage1_main_v15_generator"
    spec = importlib.util.spec_from_file_location(module_name, source)
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot load main_v15 from {source}")
...
def main() -> None:
    ...
    main_v15.apply_wavelength_cli_overrides(override)
    main_v15.configure_v15()
        ...
        for case_name in cases:
            ...
            for realization_index in range(args.repeats):
                ...
                try:
                    ...
                    rows.append(main_v15.dataset_index_row(metadata, path))
    ...
    manifest = {
        "version": "v15_stage1_wideband_stackrt_master",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "optical_backend": "Lumerical lumapi.FDTD.stackrt",
        "generator_version_in_npz": "main_v15",
        ...
        "seed_base": args.seed,
        "seed_policy": "original complete V15 case-index offset",
        "historical_pairing_guard": (
            "Filenames and seeds align with V15, but realization values are not assumed "
        ...
        "source_audit": {
            ...
            "eq99x_csv": str(main_v15.eq99x_csv_path()),
            "eq99x_csv_sha256": main_v15.V15_CONFIG["EQ99X_SOURCE_CSV_SHA256"],
            "eq99x_column": main_v15.EQ99X_COLUMN,
        },
        "instrument_scope": (
            "Digitized EQ-99X source_shape_peak_normalized, quadratic QE, Gaussian ILS, "
            "detector and ADC proxy; EQ-99X CSV hash embedded in config_json"
        ),
        "failures": failures,
    }

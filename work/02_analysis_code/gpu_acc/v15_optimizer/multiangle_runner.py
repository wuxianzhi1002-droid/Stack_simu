"""V15 Stage 3 EQ-99X single-angle versus two-angle GPU joint fitting."""
...
VERSION='v15_stage3_multiangle_joint_gpu';EXPECTED_V10='d0c5076deef971a3cce169220e37072fa8dd61a24f6a21401707f6150474b321'
...
def load_group(path,start,stop):
 with np.load(path,allow_pickle=False) as d:
  ...
  if str(scalar(d,'generator_version',''))!='main_v15_multiangle' or 'stackrt' not in str(scalar(d,'multiangle_provenance','')).lower(): raise ValueError(f'bad provenance {path.name}')
...
def main():
 ...
 output=args.output_dir.resolve();output.mkdir(parents=True,exist_ok=False);progress=output/f'v15_stage3_{args.band.replace("-","_")}_progress.jsonl';rows=[];tall=time.perf_counter()
 ...
 lines=[f'# V15 Stage 3 {args.band} EQ-99X multi-angle result','',f'- Overall: **{report["overall"]}**',f'- Groups/fits/closure: {len(paths)}/{len(rows)}/{summary["closure_passed"]}',f'- Runtime: {elapsed:.3f} s','', '| mode | Air abs mean (nm) | film MAE mean (nm) | boundary rate | sigma_min(Jw) | kappa(Jw) |','|---|---:|---:|---:|---:|---:|']

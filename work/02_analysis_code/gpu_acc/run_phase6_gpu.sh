...
#SBATCH --job-name=v10_phase6_401
#SBATCH --output=phase6_slurm_%j.out
#SBATCH --error=phase6_slurm_%j.err
set -Eeo pipefail
printf '=== V10 Phase 6 full 401-NPZ bootstrap ===\n'
...
INPUT_DIR="${TMM_PHASE6_INPUT_DIR:-$HOME/gpu_acc/04_results_and_datasets/static_stackrt_v10_20260825_232804}"
CPU_BASELINE="${TMM_PHASE6_CPU_BASELINE:-$PROJECT_DIR/cluster_runs/1482539/phase4_result.json}"
[ -d "$INPUT_DIR" ] || { printf '[FAIL] Missing input directory: %s\n' "$INPUT_DIR" >&2; exit 2; }
[ -f "$CPU_BASELINE" ] || { printf '[FAIL] Missing CPU baseline: %s\n' "$CPU_BASELINE" >&2; exit 2; }
export TMM_PHASE6_PROCESS_START_EPOCH_NS="$(date +%s%N)"
set +e
python -m v10_gpu.phase6_full_runner --input-dir "$INPUT_DIR" --output-dir "$RUN_DIR" --machine "$RUN_DIR/machine_info.json" --backend gpu --count 401 --global-forward-model fast_no_ils --global-popsize 8 --global-maxiter 40 --multistarts 8 --max-nfev 600 --random-seed 20260825 --cpu-baseline "$CPU_BASELINE" | tee "$RUN_DIR/phase6_console.json"
PHASE6_CODE=${PIPESTATUS[0]}
set -e
[ "$PHASE6_CODE" -eq 0 ] || exit "$PHASE6_CODE"
printf '[PASS] Full 401-NPZ Phase 6 finished.\n'

...
#SBATCH --job-name=v16_200_600_pair
#SBATCH --output=v16_stage3_slurm_%j.out
#SBATCH --error=v16_stage3_slurm_%j.err
set -Eeo pipefail
...
BAND="${TMM_V16_BAND:?Set TMM_V16_BAND}"
RUN_ROOT="${TMM_V16_RUN_ROOT:-/ssd/$USER/v16_stage3_runs}"
RUN_DIR="$RUN_ROOT/$SLURM_JOB_ID";CASE_COUNT="${TMM_V16_CASE_COUNT:-100}"
case "$BAND" in 200-600);; *) echo '[FAIL] unsupported band' >&2;exit 2;;esac
...
for p in paths:
 with np.load(p,allow_pickle=False) as d:
  assert d['spectra_measured'].shape==(2,30001);a=np.asarray(d['true_reflector_angles_deg'],dtype=float);assert a.shape==(2,) and np.allclose(a,[0.1,0.2],rtol=0.0,atol=1e-12),a;assert str(d['generator_version'])=='main_v16_multiangle_nonzero';c=json.loads(str(d['config_json']));assert c['SOURCE_MODEL']=='eq99x_digitized_peak_normalized' and c['EQ99X_SOURCE_CSV_SHA256']
groups=sorted(pathlib.Path(sys.argv[2]).glob('multiframe_g*.npz'));assert len(groups)==10
print('[PASS] V16 EQ-99X nonzero-angle two-angle StackRT dataset and calibration gates')
PY
...
ARGS=(--input-dir "$INPUT_DIR" --noise-calibration-dir "$CALIBRATION_DIR" --output-dir "$RUN_DIR/results" --machine "$RUN_DIR/machine_info.json" --band "$BAND" --count "$CASE_COUNT" --global-maxiter "${TMM_V16_GLOBAL_MAXITER:-40}" --max-nfev "${TMM_V16_MAX_NFEV:-600}")
[ "$CASE_COUNT" -eq 100 ]&&ARGS+=(--require-production-count)
python -m v16_optimizer.multiangle_runner_200_600 "${ARGS[@]}"

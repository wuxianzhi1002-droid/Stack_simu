...
#SBATCH --job-name=v17_ablation_r10_r49
#SBATCH --output=v17_ablation_50pairs_increment_slurm_%j.out
#SBATCH --error=v17_ablation_50pairs_increment_slurm_%j.err
set -Eeo pipefail
...
CASE_COUNT="${TMM_V17_CASE_COUNT:-400}"
case "$CASE_COUNT" in 1|400);; *) echo '[FAIL] count must be 1 or 400' >&2; exit 2;; esac
...
root=pathlib.Path(sys.argv[1]); paths=sorted(root.glob('*__static_spectrum_combined_typical_r*.npz')); assert len(paths)==400,len(paths)
counts=collections.Counter(); realizations=set()
for p in paths:
 with np.load(p,allow_pickle=False) as d:
  scenario=str(d['ablation_scenario']);counts[scenario]+=1;realizations.add(int(d['realization_index']))
...
assert len(counts)==10 and set(counts.values())=={40},counts; assert realizations==set(range(10,50)),realizations
...
print('[PASS] V17 r10-r49 StackRT ablation matrix and calibration gates',counts)
PY
...
[ "$CASE_COUNT" -eq 400 ] && ARGS+=(--require-production-count)
python -m v17_optimizer.ablation_runner_50pairs_increment "${ARGS[@]}"

"""Build compact V17 r10-r49 inputs for the 50-pair extension."""
...
def main():
    ...
    if len(sources) != 400:
        raise RuntimeError(f"expected 400 r10-r49 inputs, got {len(sources)}")
    rows = []
    ...
    manifest = {
        "version": "v17_compact_ablation_remote_inputs_r10_r49", "count": len(rows),
        "source_manifest": json.loads(source_manifest.read_text(encoding="utf-8")),
        "source_manifest_sha256": sha256(source_manifest), "fields": FIELDS, "files": rows,
        "scope": "Field-lossless V17 r10-r49 increment; merge with validated r0-r9 results for 50 pairs.",
    }

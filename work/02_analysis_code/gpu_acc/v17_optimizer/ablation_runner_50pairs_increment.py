"""V17 r10-r49 incremental GPU runner for the 50-pair extension."""
...
VERSION = "v17_combined_typical_one_factor_reduction_gpu_r10_r49"
...
def main():
    ...
    if args.require_production_count and args.count != 400:
        raise ValueError("r10-r49 production count must be 400")
    root = args.input_dir.resolve()
    all_paths = sorted(root.glob("*__static_spectrum_combined_typical_r*.npz"), key=lambda item: item.name)
    if len(all_paths) != 400:
        raise RuntimeError(f"expected complete 400-file r10-r49 source matrix, got {len(all_paths)}")
    ...
    if args.count == 400 and counts != Counter({scenario: 40 for scenario in SCENARIOS}):
    ...
    if args.count == 400 and (set(paired) != set(range(10, 50)) or any(len(items) != 10 for items in paired.values())):
    ...
    progress = output / "v17_ablation_increment_r10_r49_progress.jsonl"
    ...
    passed = len(rows) == args.count and summary["closure_passed"] == args.count and summary["resident_reused"] and all(len(values) == (40 if args.count == 400 else 1) for values in groups.values())
    report = {
        ...
        "cases": rows, "scope_guard": "V17 r10-r49 increment for 50-pair extension; single-angle 200-600 nm combined-typical paired one-factor 0.1x reduction; fixed measured angle; no multi-angle and no Angle MAP; strict GPU full ILS.",
    }
    (output / "v17_ablation_increment_r10_r49_results.json").write_text(json.dumps(safe(report), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    with (output / "v17_ablation_increment_r10_r49_table.csv").open("w", encoding="utf-8-sig", newline="") as handle:
    ...
    lines = ["# V17 combined typical one-factor reduction r10-r49 increment", "", f"- Overall: **{report['overall']}**", f"- Cases/closure: {len(rows)}/{summary['closure_passed']}", f"- Runtime: {elapsed:.3f} s", "", "| scenario | Air abs mean nm | film MAE mean nm | Air improvement vs baseline nm | film improvement vs baseline nm |", "|---|---:|---:|---:|---:|"]
    ...
    (output / "v17_ablation_increment_r10_r49_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

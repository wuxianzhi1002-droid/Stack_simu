"""GPU runner for 50 V17 simultaneous wavelength-calibration and angle-error reductions."""
...
VERSION = "v17_combined_typical_joint_calibration_angle_reduction_gpu_r0_r49"
EXPECTED_V10 = "d0c5076deef971a3cce169220e37072fa8dd61a24f6a21401707f6150474b321"
START_NM, STOP_NM, POINTS = 200.0, 600.0, 20001
SCENARIOS = ("reduce_absolute_accuracy_and_angle_measurement",)
REDUCED_KEYS = {
...
def main():
    ...
    if args.require_production_count and args.count != 50:
        raise ValueError("joint calibration-angle production count must be 50")
    root = args.input_dir.resolve()
    all_paths = sorted(root.glob("*static_spectrum_combined_typical_r*.npz"), key=lambda item: item.name)
    if len(all_paths) != 50:
        raise RuntimeError(f"expected complete 50-file joint-reduction source matrix, got {len(all_paths)}")
    ...
    if args.count == 50 and counts != Counter({scenario: 50 for scenario in SCENARIOS}):
    ...
    if args.count == 50 and (set(paired) != set(range(50)) or any(len(items) != 1 for items in paired.values())):
        raise RuntimeError("joint-reduction realization matrix incomplete")
    for realization, items in paired.items():
        item = items[0][1]
        expected_data_seed = 20260831 + 37_000_000 + realization
        if item["paired_realization_seed"] != expected_data_seed:
            raise RuntimeError(f"data-seed pairing failed at realization {realization}")
    ...
    progress = output / "v17_joint_calibration_angle_progress.jsonl"
    ...
    for index, path in enumerate(paths, 1):
        ...
        print(f"[{index}/{args.count}] {meta['ablation_scenario']} r={meta['realization_index']} Air={row['absolute_Air_error_nm']:.5g} film={row['film_MAE_nm']:.5g}", flush=True)
    ...
    passed = len(rows) == args.count and summary["closure_passed"] == args.count and summary["resident_reused"] and all(len(values) == (50 if args.count == 50 else 1) for values in groups.values())
    report = {
        ...
        "cases": rows, "scope_guard": "V17 single-angle 200-600 nm combined-typical paired simultaneous 0.1x reduction of high-order absolute wavelength calibration residual and independent angle measurement uncertainty; fixed measured angle; no multi-angle and no Angle MAP; strict GPU full ILS.",
    }
    (output / "v17_joint_calibration_angle_results.json").write_text(json.dumps(safe(report), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    with (output / "v17_joint_calibration_angle_table.csv").open("w", encoding="utf-8-sig", newline="") as handle:
    ...
    item = report["scenario_aggregates"]["reduce_absolute_accuracy_and_angle_measurement"]
    lines = [
        "# V17 simultaneous wavelength-calibration and angle-error reduction", "",
        f"- Overall: **{report['overall']}**",
        f"- Cases/closure: {len(rows)}/{summary['closure_passed']}",
        f"- Runtime: {elapsed:.3f} s", "",
        "| scenario | Air abs mean nm | film MAE mean nm |",
        "|---|---:|---:|",
        f"| reduce_absolute_accuracy_and_angle_measurement | {item['absolute_Air_error_nm']['mean']:.6g} | {item['film_MAE_nm']['mean']:.6g} |",
    ]
    (output / "v17_joint_calibration_angle_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

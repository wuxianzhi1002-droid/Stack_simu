...
#SBATCH --job-name=v11_450_gpu401
#SBATCH --output=v11_450_gpu401_slurm_%j.out
#SBATCH --error=v11_450_gpu401_slurm_%j.err

set -Eeo pipefail
printf '=== V11 450-580 nm fresh GPU-global 401-case run ===\n'
printf 'JOBID=%s\nhostname=%s\npwd=%s\nSLURM_SUBMIT_DIR=%s\nscript=%s\nbash=%s\n' \
...
finish() {
  ...
  if [ "$code" -eq 0 ]; then
    echo '[PASS] V11 450-580 nm 401-case job completed.'
  else
...
python -m v11_optimizer.wideband_gpu_global_401_runner \
  ...
  --count 401 \
  --wavelength-min-nm 450.0 \
  --wavelength-max-nm 580.0 \
  --output-prefix v11_450_580_gpu_global_401

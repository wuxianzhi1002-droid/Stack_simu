"""V16 Stage 1B long-wave-only information diagnostics without inversion."""
...
VERSION="v16_stage1b_longwave_information_only"
BANDS={"600-800":(600.0,800.0),"500-800":(500.0,800.0),"400-800":(400.0,800.0),"300-800":(300.0,800.0)}
...
def norms(a):
 x=np.linalg.norm(np.asarray(a,dtype=np.float64),axis=0);return {n:float(x[i]) for i,n in enumerate(v12.FREE_PARAMS)}
...
def main():
 ...
 closure={"rmse":float(np.sqrt(np.mean(delta*delta))),"max_abs":float(np.max(np.abs(delta))),"reference_rms":float(np.sqrt(np.mean(cj*cj))),"reference_max_abs":float(np.max(np.abs(cj)))};closure["relative_rmse"]=closure["rmse"]/max(closure["reference_rms"],np.finfo(float).tiny);closure["relative_max_abs"]=closure["max_abs"]/max(closure["reference_max_abs"],np.finfo(float).tiny);closure["pass"]=closure["rmse"]<=1e-9 and closure["max_abs"]<=1e-8 and closure["relative_rmse"]<=1e-10
 rows=[];details={}
 for label,(lo,hi) in BANDS.items():
  mask=(axis>=lo)&(axis<=hi);j=gj[mask];sigma=csigma[mask];info=whitened_information_metrics(j,sigma);raw=info["raw_information"];density=info["per_sample_information_density"];jw=j/sigma[:,None]
  row={"band":label,"wavelength_min_nm":lo,"wavelength_max_nm":hi,"wavelength_samples":int(mask.sum()),"sigma_min_Jw":raw["smallest_singular_value"],"condition_number_Jw":raw["condition_number"],"det_JwT_Jw":raw["fisher_determinant"],"det_mantissa":raw["fisher_determinant_mantissa"],"det_exponent10":raw["fisher_determinant_exponent10"],"log10_det_JwT_Jw":raw["fisher_log10_determinant"],"density_sigma_min_Jw":density["smallest_singular_value"],"density_condition_number_Jw":density["condition_number"],"density_log10_det_JwT_Jw":density["fisher_log10_determinant"]}
  rows.append(row);details[label]={"metrics":info,"whitened_column_norms":norms(jw),"per_sample_whitened_column_norms":norms(jw/np.sqrt(len(jw)))}
 after=resident_array_identity(gpu);passed=closure["pass"] and before==after and all(details[k]["metrics"]["raw_information"]["numerical_rank"]==5 for k in BANDS)
 report={"version":VERSION,"overall":"PASS" if passed else "FAIL","created_utc":datetime.now(timezone.utc).isoformat(),"configuration":{"bands_nm":{k:list(v) for k,v in BANDS.items()},"master_band_nm":[200.0,800.0],"fixed_angle_deg":FIXED_ANGLE_DEG,"evaluation_point":"true Air/HSQ/PSS/SOC/TiO2 structure","source_model":"digitized EQ-99X","information_definition":"Jw = diag(1/sigma_lambda) @ d(signal)/d(normalized [0,1]^5 parameters)","covariance_model":"diagonal single-frame detector covariance","inversion_or_optimization_performed":False,"finite_difference_batch_size":11},"truth":truth,"jacobian_audit":audit,"gpu_cpu_closure":closure,"gpu_runtime_s":gpu_s,"cpu_closure_runtime_s":cpu_s,"resident_reused":before==after,"peak_gpu_memory_gib":int(gpu.memory_stats().get("memory_pool_total_bytes",0))/1024**3,"source_npz":{"filename":paths[0].name,"sha256":sha256_file(paths[0])},"input_manifest_sha256":sha256_file(root/"simulation_manifest.json"),"noise_covariance_audit":cov["audit"],"machine":json.loads(a.machine.read_text(encoding="utf-8")) if a.machine and a.machine.is_file() else None,"bands":details,"rows":rows,"scope_guard":"Information-only truth-point Jacobian study; no DE, no least-squares, no Air/cavity-length inversion."}
 out=a.output_dir.resolve();out.mkdir(parents=True,exist_ok=False)
 (out/"v16_stage1b_longwave_information_results.json").write_text(json.dumps(v12.safe(report),ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
 with (out/"v16_stage1b_longwave_information_table.csv").open("w",encoding="utf-8-sig",newline="") as h:w=csv.DictWriter(h,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
 lines=["# V16 Stage 1B long-wave information-only result","",f"- Overall: **{report['overall']}**",f"- Fixed angle: {FIXED_ANGLE_DEG:.3f} deg; evaluated at structural truth.","- No differential evolution, local fitting, or cavity-length inversion.","- Raw metrics grow with total samples; density metrics normalize Jw by sqrt(N).","","| band (nm) | N | sigma_min(Jw) | kappa(Jw) | det(Jw^T Jw) | log10 det | density sigma_min |","|---|---:|---:|---:|---:|---:|---:|"]
 for r in rows:
  determinant=f"{r['det_mantissa']:.6g}e{r['det_exponent10']:+d}";lines.append(f"| {r['band']} | {r['wavelength_samples']} | {r['sigma_min_Jw']:.6g} | {r['condition_number_Jw']:.6g} | {determinant} | {r['log10_det_JwT_Jw']:.6g} | {r['density_sigma_min_Jw']:.6g} |")
 (out/"v16_stage1b_longwave_information_summary.md").write_text("\n".join(lines)+"\n",encoding="utf-8")
 print(json.dumps({"overall":report["overall"],"bands":rows,"gpu_runtime_s":gpu_s,"closure":closure,"output":str(out)},indent=2),flush=True)

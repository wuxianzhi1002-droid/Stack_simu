"""V15 Stage 1a GPU runner for nested bands wider than 220-580 nm.
...
baseline.  The V15 fixed measured angle remains fixed (scheme A, sigma 0.001
deg), and the strict full-ILS GPU global/local fitting contract is unchanged.

Stage 1 uses the digitized EQ-99X source curve embedded in every NPZ config.
QE, detector, ADC, and Gaussian ILS remain the audited synthetic instrument proxy.
"""
...
from .eq99x_spectrometer import (
    Eq99xCupyStrictSpectrometerBackend,
    Eq99xNumpyStrictSpectrometerBackend,
)
...
VERSION = "v15_stage1a_wideband_information"
...
def load_v15_fit_input(path: Path, config: v12.FitConfig) -> dict[str, Any]:
    previous = v12.EXPECTED_GENERATOR
    try:
        v12.EXPECTED_GENERATOR = "main_v15"
        measurement = v12.load_fit_input(path, config)
    finally:
        v12.EXPECTED_GENERATOR = previous
    generator_config = measurement["generator_config"]
    if generator_config.get("MODEL_TYPE") != "stackrt_v15":
        raise ValueError(f"V15 model type missing: {path.name}")
    if generator_config.get("SOURCE_MODEL") != "eq99x_digitized_peak_normalized":
        raise ValueError(f"EQ-99X source model missing: {path.name}")
    if not generator_config.get("EQ99X_SOURCE_CSV_SHA256"):
        raise ValueError(f"EQ-99X source hash missing: {path.name}")
    return measurement
...
def validate_master_npz(path: Path) -> None:
    with np.load(path, allow_pickle=False) as data:
        ...
        if str(np.asarray(data["generator_version"]).item()) != "main_v15":
            raise RuntimeError(f"generator is not main_v15: {path.name}")
...
def write_outputs(output_dir: Path, report: dict[str, Any]) -> None:
    prefix = f"v15_stage1_band_{report['configuration']['band_label'].replace('-', '_')}"
    ...
    lines = [
        f"# V15 Stage 1a band {report['configuration']['band_label']} GPU result",
        "",
        ...
        "- Source: digitized EQ-99X curve; QE/detector/ADC remain synthetic proxies",
...
def main() -> None:
    ...
    progress = output_dir / f"v15_stage1_band_{args.band.replace('-', '_')}_progress.jsonl"
    rows = []
    ...
    report = {
        ...
        "configuration": {
            **asdict(config),
            ...
            "vectorized": True,
            "updating": "deferred",
            "fit_objective_weighting": "unchanged V15 robust spectral residual",
        ...
        "cases": rows,
        "scope_guard": (
            "V15 Stage 1a only; all candidate bands contain 220-580 nm; one paired "
            "200-800 nm local StackRT master dataset; typical only; fixed measured Angle "
            "sigma=0.001 deg; no Angle MAP; V15 synthetic instrument proxy is not measured "

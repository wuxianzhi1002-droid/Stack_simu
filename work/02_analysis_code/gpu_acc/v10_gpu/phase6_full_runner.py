"""Phase 6 full-dataset execution of the frozen Phase 5 GPU local-fit integration."""
...
def backend_source_hash()->dict:
    ...
    files=[package/"spectrometer/cupy_backend.py",package/"backend/cupy_backend.py",package/"jacobian/batched.py",package/"jacobian/contract.py",package/"phase5_cache.py",package/"phase5_runner.py",Path(__file__).resolve()]
...
def append_progress(output:Path,row:dict):
    output.mkdir(parents=True,exist_ok=True)
    payload=flatten_case(row) if "gpu_optimizer" in row else {"index":row.get("index"),"filename":row.get("filename"),"seed":row.get("seed"),"error":row.get("error")}
    with (output/"phase6_401npz_progress.jsonl").open("a",encoding="utf-8",newline="\n") as f:
        f.write(json.dumps(payload,ensure_ascii=False,allow_nan=False,default=json_default,separators=(",",":"))+"\n")
        f.flush()
        os.fsync(f.fileno())

def write_outputs(output:Path,report:dict):
    output.mkdir(parents=True,exist_ok=True)
    (output/"phase6_401npz_results.json").write_text(json.dumps(report,ensure_ascii=False,indent=2,allow_nan=False,default=json_default)+"\n",encoding="utf-8",newline="\n")
    rows=[flatten_case(r) for r in report["cases"]]; fields=list(rows[0].keys()) if rows else []
    with (output/"phase6_401npz_table.csv").open("w",encoding="utf-8-sig",newline="") as f:
        w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(rows)
    p=report["performance"]; a=report["acceptance"]
    lines=["# Phase 6 formal V10 GPU local-fit integration: full 401 NPZ","",f"- Overall: **{'PASS' if a['pass'] else 'FAIL'}**",f"- Effective backend: `{report['backend']['effective']}`",f"- Selected/completed: `{report['selection']['selected_count']}/{len(report['cases'])}`",f"- Formal optimizer non-convergence outcomes: {a['optimizer_nonconvergence_count']}/401",f"- GPU optimization successes (formal non-convergence is recorded, not fatal): `{a['optimization_success_count']}/401`",f"- Same-parameter CPU/GPU closure passes: `{a['closure_pass_count']}/401`",f"- One GPU backend initialization: `{report['backend']['initialization_count']}`",f"- Total measured 401-NPZ runtime: `{p['total_401npz_runtime_s']:.3f} s`",f"- Mean/median optimizer runtime per NPZ: `{p['mean_optimizer_runtime_s']:.3f}` / `{p['median_optimizer_runtime_s']:.3f} s`",f"- Total GPU optimizer runtime: `{p['total_gpu_optimizer_runtime_s']:.3f} s`",f"- Effective throughput: `{p['effective_throughput_npz_per_min']:.3f} NPZ/min`",f"- Measured projection for 401 NPZ: `{p['estimated_401_runtime_s']:.3f} s` (`{p['estimated_401_runtime_hours']:.3f} h`)",f"- Process/CUDA startup: `{p['process_cuda_startup_s']:.3f} s`; backend init: `{p['backend_initialization_s']:.3f} s`; first warmup: `{p['first_warmup_s']:.3f} s`",f"- Existing CPU baseline local optimizer: `{p['existing_cpu_baseline_optimizer_s']:.3f} s`; startup-amortized Phase 5 GPU mean per local attempt: `{p['startup_amortized_gpu_runtime_per_local_attempt_s']:.3f} s`; reference speedup: `{p['speedup_vs_existing_cpu_local_attempt']:.2f}x`","","## Per-NPZ results","","| # | filename | success | exact RMSE | closure RMSE | max abs | batches | hits/misses | optimizer s | Air error nm | film MAE nm | angle error deg |","|---:|---|:---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    ...
    lines += ["","## Acceptance","",f"- A, all 401 files processed without GPU/runtime failure: `{a['A_all_files_processed_without_runtime_failure']}`",f"- B, all successful-fit same-parameter strict closures pass: `{a['B_all_successful_same_parameter_closures_pass']}`",f"- C, formal semantics/freeze guards pass: `{a['C_formal_semantics_preserved']}`",f"- D/E, backend reused and initialized once: `{a['D_backend_reused']}/{a['E_no_per_npz_reinitialization']}`",f"- F, final CPU strict verification recorded for every successful fit: `{a['F_final_cpu_verification_complete_for_successful_fits']}`",f"- G, materially faster than existing CPU local baseline after amortization: `{a['G_material_speedup']}`","","CPU/GPU optimizer endpoint equality is not an acceptance criterion. Scientifically poor fits are reported as inversion issues when same-parameter strict closure passes.","","## Scope","","Exactly 401 valid sorted NPZ files were processed in one Python process and one initialized GPU backend. No JAX, autodiff, multi-GPU, parameter change, physics change, or formal V10 source edit was performed."]
    if report.get("fatal_error"):lines += ["","## Fatal error","",f"`{report['fatal_error']}`"]
    (output/"phase6_401npz_summary.md").write_text("\n".join(lines)+"\n",encoding="utf-8",newline="\n")

def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--input-dir",type=Path,required=True);parser.add_argument("--output-dir",type=Path,required=True);parser.add_argument("--machine",type=Path);parser.add_argument("--pattern",default="static_spectrum_*.npz");parser.add_argument("--backend",choices=["cpu","gpu"],default="gpu");parser.add_argument("--allow-cpu-fallback",action="store_true");parser.add_argument("--count",type=int,default=401);parser.add_argument("--global-forward-model",choices=["full_ils","fast_no_ils"],default="fast_no_ils");parser.add_argument("--global-popsize",type=int,default=8);parser.add_argument("--global-maxiter",type=int,default=40);parser.add_argument("--multistarts",type=int,default=8);parser.add_argument("--max-nfev",type=int,default=600);parser.add_argument("--random-seed",type=int,default=20260825);parser.add_argument("--cpu-baseline",type=Path);args=parser.parse_args()
    if args.count!=401: raise ValueError("Phase 6 full-dataset stage is locked to exactly 401 NPZ files")
    process_epoch_ns=int(os.environ.get("TMM_PHASE6_PROCESS_START_EPOCH_NS",_PROCESS_IMPORT_STARTED_NS)); process_startup_s=(time.time_ns()-process_epoch_ns)/1.0e9
    ...
    for index,(path,measurement,sampling) in enumerate(selected):
        ...
        try:
            ...
            if not fit["success"]:
                best=fit["attempts"][0]
                record=matching_adapter_record(best,adapter.calls) if adapter else None
                optimizer_stats=cache.snapshot() if cache else {"residual_calls":None,"jacobian_calls":None,"actual_gpu_batch_evaluations":None,"cache_hits":None,"cache_misses":None}
                ...
                nonconverged={
                    ...
                    "global_runtime_s":float(fit["global_runtime_s"]),
                    "total_formal_fit_runtime_s":float(fit["runtime_s"]),
                    "gpu_optimizer":{
                        "success":False,
                        ...
                        "exact_RMSE":float(best["rmse_reflectance"]),
                        "nfev":int(best["nfev"]),
                        "njev":None if record is None else record["njev"],
                        "boundary_hits":[],
                        "rank":None,
                        "rank_diagnostics":None,
                        "runtime_s":float(sum(r["runtime_s"] for r in adapter.calls)) if adapter else float(fit["local_runtime_s"]),
                        "formal_local_stage_runtime_s":float(fit["local_runtime_s"]),
                        "best_local_attempt_runtime_s":None if record is None else record["runtime_s"],
                        "local_attempt_count":len(fit["attempts"]),
                    },
                    "gpu_backend":{**optimizer_stats,"response_backend_id":None if gpu is None else id(gpu),"tmm_backend_id":None if gpu is None else id(gpu.tmm_backend)},
                    "optimizer_nonconvergence":{
                        "classification":"formal optimizer outcome; no fitted solution was ranked",
                        "attempt_statuses":[int(a["status"]) for a in fit["attempts"]],
                        "attempt_messages":[str(a["message"]) for a in fit["attempts"]],
                        "attempt_nfev":[int(a["nfev"]) for a in fit["attempts"]],
                    },
                    "truth_loaded_after_fit_and_ranking":True,
                    "noise_audit":noise,
                    "local_attempts":adapter.calls if adapter else [],
                    "case_wall_runtime_s":time.perf_counter()-case_started,
                }
                cases.append(nonconverged)
                append_progress(args.output_dir,nonconverged)
                print(f"[{index+1}/401] {path.name} success=False closure=N/A local={fit['local_runtime_s']:.3f}s batches={optimizer_stats.get('actual_gpu_batch_evaluations')}",flush=True)
                continue
            ...
            cases.append(result);append_progress(args.output_dir,result);print(f"[{index+1}/401] {path.name} success={best['success']} closure={closure_pass} local={fit['local_runtime_s']:.3f}s batches={optimizer_stats.get('actual_gpu_batch_evaluations')}",flush=True)
            if not closure_pass:fatal=f"Same-parameter strict closure failed for {path.name}";break
        except Exception as exc:
            fatal=f"{path.name}: {type(exc).__name__}: {exc}";error_row={"index":index+1,"filename":path.name,"path":str(path.resolve()),"npz_sha256":sha256_file(path),"seed":seed,"error":fatal};cases.append(error_row);append_progress(args.output_dir,error_row);break
    ...
    completed_optimizers=[c for c in cases if "gpu_optimizer" in c];successful=[c for c in completed_optimizers if c.get("gpu_optimizer",{}).get("success")];optimizer_times=[c["gpu_optimizer"]["runtime_s"] for c in completed_optimizers];attempt_times=[a["runtime_s"] for c in completed_optimizers for a in c.get("local_attempts",[])]

...
for p in paths:
 with np.load(p,allow_pickle=False) as d:
  assert d['spectra_measured'].shape==(2,30001);assert (lambda a: a.shape==(2,) and 0.0<a[0]<a[1]<=0.2)(np.asarray(d['true_reflector_angles_deg'],dtype=float));assert str(d['generator_version'])=='main_v16_multiangle_nonzero';c=json.loads(str(d['config_json']));assert c['SOURCE_MODEL']=='eq99x_digitized_peak_normalized' and c['EQ99X_SOURCE_CSV_SHA256']
...
python -m v16_optimizer.multiangle_runner "${ARGS[@]}"

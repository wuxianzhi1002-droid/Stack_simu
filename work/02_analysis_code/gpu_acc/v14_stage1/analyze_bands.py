"""Aggregate and compare the five paired V14 Stage 1a band reports."""
...
def load_report(path: Path) -> dict[str, Any]:
    ...
    if report.get("version") != "v14_stage1a_wideband_information":
        raise ValueError(f"not a V14 Stage 1a report: {path}")
...
def main() -> None:
    ...
    write_csv(output / "v14_stage1_band_summary.csv", summary_rows)
    write_csv(output / "v14_stage1_noise_case_summary.csv", noise_rows)
    write_csv(output / "v14_stage1_paired_vs_220_580.csv", paired_rows)
    ...
    analysis = {
        "version": "v14_stage1a_paired_band_analysis",
    ...
    (output / "v14_stage1_analysis.json").write_text(
        json.dumps(analysis, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )

    lines = [
        "# V14 Stage 1a：220–580 nm 基线之外的波段拓宽",
        "",
        "## 审计口径",
        "",
        ...
        "- 当前源功率/QE/探测器为 V12 仪器链代理，不宣称为实测硬件曲线。",
        "",
        "## 总体结果",
        "",
    ...
    (output / "v14_stage1_analysis.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

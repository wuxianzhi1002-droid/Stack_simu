"""Build V14 Stage 1 detector frames over the 200-800 nm master band.

Optical spectra and expected electron arrays must already come from local
``main_v12.py --backend api`` StackRT generation.  This script only resamples
...
def validate_source(data, source: Path) -> dict:
    ...
    if str(scalar(data, "generator_version", "")) != "main_v12":
        raise RuntimeError(f"source is not main_v12: {source.name}")
    ...
    if config.get("MODEL_TYPE") != "stackrt_v12":
    ...
    copied.update(
        {
            ...
            "multiframe_provenance": np.asarray(
                "V14 Stage 1 detector frames resampled from main_v12 StackRT "
...
def main() -> None:
    ...
    manifest = {
        ...
        "version": "v14_stage1_detector_calibration",

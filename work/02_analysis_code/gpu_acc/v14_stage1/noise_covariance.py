...
def estimate_diagonal_sigma(
    ...
    for path in paths:
        with np.load(path, allow_pickle=False) as data:
            ...
            if generator != "main_v12" or "StackRT" not in provenance:
                raise ValueError(f"calibration provenance is not V12 StackRT-derived: {path.name}")
    ...
    audit = {
        ...
        "files": file_rows,
        "scope": (
            "V12 instrument-chain proxy: wavelength-dependent detector/photon/read/"

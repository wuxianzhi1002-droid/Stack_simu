"""V12 single-frame dataset generator with an independent angle measurement.

V12 owns a complete configuration.  It reuses the audited V10 physical/noise
implementation through a private module instance, but never reads or mutates
``main_v10.CONFIG`` in the normal imported module.

Default V12 contract:
...
def _load_private_v10_kernel():
    ...
    module_name = "_stackrt_main_v10_kernel_for_v12"
...
# ---------------------------------------------------------------------------
# Complete V12-owned config.  Do not derive this dictionary from V10 CONFIG.
# ---------------------------------------------------------------------------
V12_CONFIG = {
    "MODEL_TYPE": "stackrt_v12",
...
def validate_v12_config() -> None:
    start_nm = float(V12_CONFIG["WAVELENGTH_START_UM"]) * 1000.0
    stop_nm = float(V12_CONFIG["WAVELENGTH_STOP_UM"]) * 1000.0
    sampling_nm = float(V12_CONFIG["OUTPUT_SAMPLING_NM"])
    internal_step_nm = float(V12_CONFIG["INTERNAL_WAVELENGTH_STEP_NM"])

    if not np.isfinite(start_nm) or not np.isfinite(stop_nm):
        raise ValueError("V12 wavelength range must be finite.")
    if not np.isfinite(sampling_nm) or sampling_nm <= 0.0:
        raise ValueError("V12 output sampling must be positive.")
    if not np.isfinite(internal_step_nm) or internal_step_nm <= 0.0:
        raise ValueError("V12 internal step must be positive.")
    ...
    V12_CONFIG["OUTPUT_SAMPLE_POINTS"] = points
...
def realize_noise_v12(
    ...
    nominal_angle = float(V12_CONFIG["REFLECTOR_ANGLE_NOMINAL_DEG"])
    angle_config = V12_CONFIG["ANGLE_MEASUREMENT"]
...
class V12DatasetGenerator(_kernel.StaticDatasetGenerator):
    """V12 derivative of the V10 dataset generator.

    It keeps the V10 forward model and signal chain, but overwrites the
    angle contract to the V12 independent measurement model.
    """
    def save_one(self, data: dict) -> Path:
        """Save the kernel payload, then atomically add V12 inversion inputs."""
        ...
        temporary = path.with_name(path.name + ".v12tmp")
        try:
...
def configure_v12() -> None:
    """Configure only the private kernel module; public main_v10 stays untouched."""
    validate_v12_config()
    _kernel.CONFIG = copy.deepcopy(V12_CONFIG)
    _kernel.GENERATOR_VERSION = GENERATOR_VERSION
    _kernel.WAVELENGTH_ACCURACY_SPEC_NM = float(
        V12_CONFIG["SPECTROMETER"]["WAVELENGTH_ACCURACY_SPEC_NM"]
    )
    _kernel.realize_noise = realize_noise_v12
...
def apply_wavelength_cli_overrides(args: argparse.Namespace) -> None:
    ...
    V12_CONFIG["WAVELENGTH_START_UM"] = start_nm / 1000.0
    V12_CONFIG["WAVELENGTH_STOP_UM"] = stop_nm / 1000.0
    V12_CONFIG["OUTPUT_SAMPLING_NM"] = sampling_nm
    V12_CONFIG["OUTPUT_SAMPLE_POINTS"] = points
    validate_v12_config()
...
def wavelength_budget_audit() -> dict:
    ...
    return {
        ...
        "angle_contract": V12_CONFIG["ANGLE_MEASUREMENT"],
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="V12 dataset generator with a fixed independent angle measurement."
    )
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else output_root / f"static_stackrt_v12_{timestamp}"
    )
    output_dir.mkdir(parents=True, exist_ok=False)

    generator = V12DatasetGenerator(args.backend, output_dir)
    rows: list[dict] = []
    failures: list[dict] = []
    ...
    manifest = {
        ...
        "wavelength_axis_scenario": V12_CONFIG["WAVELENGTH_AXIS_SCENARIO"],
        "reported_axis_is_fixed": True,
        "axis_error_sign_convention": V12_CONFIG["SPECTROMETER"][
            "AXIS_ERROR_SIGN_CONVENTION"
        ],
        "wavelength_accuracy_spec_nm": V12_CONFIG["SPECTROMETER"][

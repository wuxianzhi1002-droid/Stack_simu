...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scenarios",
        nargs="+",
        choices=["clean", "uniform_pm_nm", "random_uniform_nm", "gaussian_sigma_nm", "gaussian_sigma_nm_clipped"],
        default=["gaussian_sigma_nm", "random_uniform_nm"],
        help=(
            "clean uses the original labels. uniform_pm_nm adds U(-u,+u) "
            "noise to the used train labels. gaussian_sigma_nm adds unclipped N(0,u) noise. gaussian_sigma_nm_clipped clips "
            "N(0,u) to [-u,+u]."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[10.0],
        help="Uncertainty half-widths/sigmas to compare, in nm. Defaults to 10 nm for v4.",
    )
...
def make_output_dir() -> Path:
    ...
    output_dir = SCRIPT_DIR / f"residual_mlp_simple_pca_label_uncertainty_v4_{stamp}"
...
def scenario_label_noise(
    ...
    if scenario in {"uniform_pm_1nm", "uniform_pm_nm", "random_uniform_nm"}:
        return rng.uniform(-uncertainty_nm, uncertainty_nm, size=size).astype(np.float64)
    if scenario == "gaussian_sigma_nm":
        return rng.normal(0.0, uncertainty_nm, size=size).astype(np.float64)
    if scenario in {"gaussian_sigma_1nm_clipped", "gaussian_sigma_nm_clipped"}:

...
def plot_method_comparison(metrics: dict, output_dir: str):
    ...
    mae_values = [methods[method]['delta_MAE_nm'] for method in method_names]
    rmse_values = [methods[method]['delta_RMSE_nm'] for method in method_names]
    ...
    for method_name in method_names:
        print(f"\n{method_name}:")
        print(f"  MAE:  {methods[method_name]['delta_MAE_nm']:.2f} nm")
        print(f"  RMSE: {methods[method_name]['delta_RMSE_nm']:.2f} nm")
    
    improvement_mae = methods['base scalar']['delta_MAE_nm'] - methods['more feature']['delta_MAE_nm']
    improvement_rmse = methods['base scalar']['delta_RMSE_nm'] - methods['more feature']['delta_RMSE_nm']
    improvement_mae_pct = (improvement_mae / methods['base scalar']['delta_MAE_nm']) * 100
    improvement_rmse_pct = (improvement_rmse / methods['base scalar']['delta_RMSE_nm']) * 100
    
    print(f"\nMore Feature 相比 Base Scalar:")
    print(f"  MAE 改善:  {improvement_mae:.2f} nm ({improvement_mae_pct:+.2f}%)")
    print(f"  RMSE 改善: {improvement_rmse:.2f} nm ({improvement_rmse_pct:+.2f}%)")
    
    print("\n" + "="*70)


def main():
    metrics_path = r"D:\激光干涉仪\simulation\Lumerical_simulation\STACK_simu\01_Lumerical_Workflow\ML try\Residual MLP\residual_mlp_simple_pca_20260623_161143\metrics.json"

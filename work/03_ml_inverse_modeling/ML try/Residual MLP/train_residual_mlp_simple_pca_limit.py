...
# ============================================================================
# This PCA limit copy trains exactly two MLP models:
#   1. more_feature_pca50_nominal_thickness: nominal/fuzzy thickness plus MORE_FEATURE_NAMES plus PC1~PC50.
#   2. more_feature_pca50_true_thickness: true thickness plus MORE_FEATURE_NAMES plus PC1~PC50.
#
# To change the non-PCA extra features, edit only MORE_FEATURE_NAMES. Supported names are:
...
TRUE_THICKNESS_FEATURE_NAMES = [f"{layer}_true_nm" for layer in DEPLOYABLE_LAYER_ORDER]

# Edit this list to change the non-PCA extra inputs for both compared models.
MORE_FEATURE_NAMES = [
...
class TrainConfig:
    ...
    true_more_feature_names: list[str]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train PCA50 nominal-thickness and true-thickness more_feature Residual MLP limit models."
    )
...
def make_output_dir() -> Path:
    ...
    output_dir = SCRIPT_DIR / f"residual_mlp_simple_pca_limit_{stamp}"
...
def read_true_thickness(data: np.lib.npyio.NpzFile) -> np.ndarray:
    """Read sample-level true film thickness in deployable layer order."""
    ...
    if "film_true_nm" not in data.files:
        raise KeyError(
            "The limit experiment needs film_true_nm or flattened *_true_nm fields."
        )
    if "layer_names" not in data.files:
        raise KeyError("Using film_true_nm requires layer_names.")
...
def validate_unique_feature_names(feature_names: list[str]) -> None:
    duplicates = sorted({name for name in feature_names if feature_names.count(name) > 1})
    if duplicates:
        raise ValueError(f"Input feature list contains duplicated names: {duplicates}")


def validate_input_feature_names(feature_names: list[str]) -> None:
    validate_unique_feature_names(feature_names)
...
def train_mlp_method(
    ...
    uses_true_thickness: bool = False,
) -> dict:
    """
    训练一个 MLP 方法。
    ...
    joblib.dump(
        {
            ...
            "target_std_nm": y_std,
            "uses_true_thickness": uses_true_thickness,
...
def save_plots(
    ...
    plot_methods = [
        ...
        for name in [
            ...
            "more_feature_pca50_true_thickness",
        ]
        if name in results
    ]
    selected_name = "more_feature_pca50_true_thickness" if "more_feature_pca50_true_thickness" in results else plot_methods[-1]
    ...
    plt.xlabel("L_fft_um")
    plt.ylabel(f"{selected_name} error (nm)")
    ...
    display_name = {
        "more_feature_pca50_nominal_thickness": "more feature PCA50 nominal",
        "more_feature_pca50_true_thickness": "more feature PCA50 true",
    }
    display_labels = [display_name.get(label, label) for label in labels]
...
def write_summary_report(
    ...
    """Write the PCA50 nominal-vs-true-thickness comparison report."""

    results = metrics_payload["results"]
    nominal_key = "more_feature_pca50_nominal_thickness"
    true_key = "more_feature_pca50_true_thickness"
    nominal_test = results[nominal_key]["metrics"]["test"]
    true_test = results[true_key]["metrics"]["test"]
    true_better = true_test["cavity_RMSE_nm"] < nominal_test["cavity_RMSE_nm"]
    nominal_names = results[nominal_key]["feature_names"]
    true_names = results[true_key]["feature_names"]
    ...
    lines = [
        ...
        "## Model Definitions",
        "",
        "- `more_feature_pca50_nominal_thickness`: " + ", ".join(nominal_names) + ".",
        "- `more_feature_pca50_true_thickness`: " + ", ".join(true_names) + ".",
    ...
    lines += [
        ...
        (
            ...
            if true_better
            else "- `more_feature_pca50_true_thickness` does not have lower test RMSE than `more_feature_pca50_nominal_thickness`."
        ),
        f"- nominal/fuzzy thickness test: {fmt_metric(nominal_test)}",
        f"- true thickness test: {fmt_metric(true_test)}",
        f"- High-correlation feature pairs in nominal/fuzzy model inputs: {high_corr_count}.",
...
def main() -> None:
    ...
    if abs(args.train_ratio + args.val_ratio + args.test_ratio - 1.0) > 1e-8:
        raise ValueError("--train-ratio + --val-ratio + --test-ratio ???? 1?")

    pca_names = pca_feature_names(PCA_COMPONENTS) if USE_PCA_FEATURES else []
    nominal_more_feature_names = BASE_FEATURE_NAMES.copy() + MORE_FEATURE_NAMES + pca_names
    true_more_feature_names = ["L_fft_um"] + TRUE_THICKNESS_FEATURE_NAMES + MORE_FEATURE_NAMES + pca_names
    validate_input_feature_names(nominal_more_feature_names)
    validate_input_feature_names(["L_fft_um"] + MORE_FEATURE_NAMES + pca_names)
    validate_unique_feature_names(true_more_feature_names)
    ...
    config = TrainConfig(
        ...
        base_feature_names=[],
        more_feature_names=nominal_more_feature_names,
        true_more_feature_names=true_more_feature_names,
    ...
    print(f"????: {output_dir}")
    print(f"nominal/fuzzy PCA50 more_feature features: {nominal_more_feature_names}")
    print(f"true-thickness PCA50 more_feature features: {true_more_feature_names}")
    print(f"PCA enabled: {USE_PCA_FEATURES}, components={PCA_COMPONENTS if USE_PCA_FEATURES else 0}")
    print("?????? PCA50 more_feature ???/??????????????")

    with np.load(dataset_path, allow_pickle=True) as data:
        nominal_nm = read_nominal_thickness(data)
        true_thickness_nm = read_true_thickness(data)
        ...
        nominal_more_feature_matrix = resolve_feature_matrix(data, nominal_more_feature_names, nominal_nm)
        extra_feature_names = MORE_FEATURE_NAMES + pca_names
        if extra_feature_names:
            extra_feature_matrix = resolve_feature_matrix(data, extra_feature_names, nominal_nm)
        else:
            extra_feature_matrix = np.empty((nominal_nm.shape[0], 0), dtype=np.float32)
        true_more_feature_matrix = np.column_stack(
            [l_fft_um.reshape(-1, 1), true_thickness_nm, extra_feature_matrix]
        ).astype(np.float32, copy=False)

        valid_mask = build_valid_mask(data, nominal_more_feature_matrix, y_delta_nm)
        valid_mask &= np.all(np.isfinite(true_thickness_nm), axis=1)
        ...
        if dataset_split is not None:
            ...
            split_source = "generated_from_args"
        ...
        def build_nominal_more_features(indices: np.ndarray) -> tuple[np.ndarray, list[str], None]:
            return nominal_more_feature_matrix[indices], nominal_more_feature_names.copy(), None

        def build_true_more_features(indices: np.ndarray) -> tuple[np.ndarray, list[str], None]:
            return true_more_feature_matrix[indices], true_more_feature_names.copy(), None

        results["more_feature_pca50_nominal_thickness"] = train_mlp_method(
            "more_feature_pca50_nominal_thickness",
            build_nominal_more_features,
            indices_by_split,
            y_delta_nm,
            output_dir,
            args,
        ...
        results["more_feature_pca50_true_thickness"] = train_mlp_method(
            "more_feature_pca50_true_thickness",
            build_true_more_features,
            indices_by_split,
            y_delta_nm,
            output_dir,
            args,
            uses_true_thickness=True,
        )

        nominal_train_x = nominal_more_feature_matrix[indices_by_split["train"]]
        corr_info = save_correlation_files(
            output_dir,
            nominal_train_x,
            nominal_more_feature_names,
            rng,
            args.max_corr_rows,
        )
        del nominal_train_x
        ...
        metrics_payload = {
            ...
            "dataset": {
                ...
                "target": "delta_L_nm",
                "input_policy": "Compare PCA50 more_feature inputs with nominal/fuzzy thickness versus sample-level true thickness. No model uses film_delta, cavity_true_um, target, quadratic, or interaction inputs.",
                "more_feature_names": nominal_more_feature_names,
                "true_more_feature_names": true_more_feature_names,
            },
            "split": {

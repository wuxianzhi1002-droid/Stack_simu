...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the simple PCA residual MLP under artificial label "
            "measurement uncertainty without modifying the source workflow."
        )
    )
    ...
    parser.add_argument("--label-uncertainty-nm", type=float, default=1.0)
    return parser.parse_args()


def make_output_dir() -> Path:
    stamp = time.strftime("%Y%m%d_%H%M%S")
    output_dir = SCRIPT_DIR / f"residual_mlp_simple_pca_label_uncertainty_{stamp}"
...
def train_mlp_method(
    ...
    artifact_name = f"{scenario}__{method_name}"
    ...
    joblib.dump(
        {
            ...
            "label_uncertainty_nm": float(args.label_uncertainty_nm),
...
def save_comparison_plots(
    ...
    plt.xticks(x, labels, rotation=25, ha="right", fontsize=8)
    plt.ylabel("test samples within +/-1 nm (%)")
    plt.title("Fraction Inside Label-Uncertainty Band")
    plt.grid(True, axis="y", alpha=0.25)
    plt.tight_layout()
    path = output_dir / "02_within_1nm_fraction.png"
    plt.savefig(path, dpi=200, bbox_inches="tight")
    plt.close()
    paths["within_1nm_fraction"] = str(path)
...
def write_summary_report(
    ...
    clean_more = metrics_payload["results"].get("clean__more_feature")
    uniform_more = metrics_payload["results"].get("uniform_pm_1nm__more_feature")
    conclusion = "No clean/uniform more_feature pair was trained."
    if clean_more is not None and uniform_more is not None:
        clean_rmse = clean_more["metrics_clean_reference"]["test"]["cavity_RMSE_nm"]
        uniform_rmse = uniform_more["metrics_clean_reference"]["test"]["cavity_RMSE_nm"]
        conclusion = (
            "uniform_pm_1nm more_feature changed clean-reference test RMSE by "
            f"{uniform_rmse - clean_rmse:+.3f} nm versus clean-label training."
        )
...
def main() -> None:
    ...
    if args.label_uncertainty_nm < 0:
        raise ValueError("--label-uncertainty-nm must be non-negative.")
    ...
    print(f"label uncertainty: +/-{args.label_uncertainty_nm} nm")
    ...
    with np.load(dataset_path, allow_pickle=True) as data:
        ...
        for scenario in args.scenarios:
            ...
            scenario_noise_stats[scenario] = noise_stats(train_noise)
            ...
            for method_name in args.methods:
                result = train_mlp_method(
                    ...
                    args=args,
                    train_noise_stats=scenario_noise_stats[scenario],
                )
        ...
        metrics_payload = {
            ...
            "label_noise": {
                ...
                "uniform_pm_1nm_expected_noise_rmse_nm": float(args.label_uncertainty_nm / math.sqrt(3.0)),
            },
            "pca": pca_metadata,
            "results": {
                name: dict(result)

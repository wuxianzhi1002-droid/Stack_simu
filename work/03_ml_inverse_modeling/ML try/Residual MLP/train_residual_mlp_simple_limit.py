...
# ============================================================================
# This copy trains exactly three MLP models:
#   1. base_scalar: BASE_FEATURE_NAMES only.
#   2. more_feature: BASE_FEATURE_NAMES plus MORE_FEATURE_NAMES.
#   3. more_feature_true_thickness: L_fft_um, true film thickness, plus MORE_FEATURE_NAMES.
#
# To change the second and third models, edit only MORE_FEATURE_NAMES. Supported names are:
...
#
# Example command:
# python "01_Lumerical_Workflow\ML try\Residual MLP\train_residual_mlp_simple.py" ^
...
# Edit this list to change the extra inputs for both more_feature models.
MORE_FEATURE_NAMES = [
...
def make_output_dir() -> Path:
    ...
    output_dir = SCRIPT_DIR / f"residual_mlp_simple_limit_{stamp}"
...
def save_plots(
    ...
    plot_methods = [
        name
        for name in ["base_scalar", "more_feature", "more_feature_true_thickness"]
        if name in results
    ]
    selected_name = "more_feature_true_thickness" if "more_feature_true_thickness" in results else plot_methods[-1]
    ...
    display_name = {
        ...
        "more_feature": "more feature",
        "more_feature_true_thickness": "more feature true thickness",
    }
...
def write_summary_report(
    ...
    lines = [
        ...
        "- `more_feature_true_thickness`: " + ", ".join(limit_names) + ".",
    ...
    lines += [
        ...
        (
            ...
            if limit_better
            else "- `more_feature_true_thickness` does not have lower test RMSE than `more_feature`."
        ),
        f"- base_scalar test: {fmt_metric(base_test)}",
        f"- more_feature test: {fmt_metric(more_test)}",
        f"- more_feature_true_thickness test: {fmt_metric(limit_test)}",
...
def main() -> None:
    ...
    true_more_feature_names = ["L_fft_um"] + TRUE_THICKNESS_FEATURE_NAMES + MORE_FEATURE_NAMES
    validate_input_feature_names(base_feature_names)
    validate_input_feature_names(more_feature_names)
    validate_input_feature_names(["L_fft_um"] + MORE_FEATURE_NAMES)
    ...
    with np.load(dataset_path, allow_pickle=True) as data:
        ...
        if MORE_FEATURE_NAMES:
            extra_feature_matrix = resolve_feature_matrix(data, MORE_FEATURE_NAMES, nominal_nm)
        else:
        ...
        results["more_feature_true_thickness"] = train_mlp_method(
            "more_feature_true_thickness",
        ...
        metrics_payload = {
            ...
            "dataset": {
                ...
                "target": "delta_L_nm",
                "input_policy": "base_scalar and more_feature use nominal thickness; more_feature_true_thickness uses sample-level true thickness as a limit experiment. No model uses film_delta, cavity_true_um, target, quadratic, or interaction inputs.",
                "base_feature_names": base_feature_names,
                "more_feature_names": more_feature_names,
                "true_more_feature_names": true_more_feature_names,
            },

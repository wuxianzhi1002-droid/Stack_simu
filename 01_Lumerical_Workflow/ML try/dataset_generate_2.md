...
本文件用于记录当前项目中 `01_Lumerical_Workflow/ML try/` 目录下神经网络 / 残差模型数据集的最新仿真范围、保存格式、光谱特征保存策略和续跑要求。
...
```text
01_Lumerical_Workflow/ML try/
```

不要输出到：

```text
01_Lumerical_Workflow/stackrt_result/
```
...
```text
01_Lumerical_Workflow/ML try/build_nn_cavity_dataset.py
01_Lumerical_Workflow/ML try/resume_nn_cavity_scalar_results.py
```

建议新的输出目录命名为：

```text
01_Lumerical_Workflow/ML try/nn_cavity_spectral_features_YYYYMMDD_HHMMSS/
```

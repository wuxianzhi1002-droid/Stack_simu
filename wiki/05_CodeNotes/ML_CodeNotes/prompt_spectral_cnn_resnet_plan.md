...
在已经证明 `PCA(spectra_norm_ds)` 能显著提升 test 指标的前提下，进一步训练基于归一化、降采样光谱序列的深度模型：
...
原始光谱在实现中按 **20000 点** 保存或处理；如果数学定义同时包含 `200 nm` 和 `600 nm` 两端点，则理论点数为 `(600-200)/0.02 + 1 = 20001`，实际生成流程会通过端点约定或裁剪统一为 20000 点。

当前 CNN 数据集使用的是原始光谱经过 `spectra_downsample_factor = 10` 降采样后的 `spectra_norm_ds`，因此：

- 原始光谱长度：`20000`
- 降采样因子：`10`
- 实际 CNN 输入长度：`2000`

本计划中的模型训练默认使用数据集现有的 `spectra_norm_ds`，即 **2000 点输入**，不要求 重新加载未降采样的完整光谱。

### 1.2 模型输入

#### 光谱输入（序列分支）

- 默认字段：`spectra_norm_ds`
- 可选字段：未降采样的 `spectra_norm_full`，仅用于后续独立消融实验
- 当前默认输入：单个样本长度 `2000`
- 形状：
  - 数据集数组：`(N, 2000)`
  - 送入 PyTorch：`(B, 1, 2000)`

说明：光谱已经完成 per-spectrum normalization 和 10 倍降采样，因此 CNN 分支直接使用归一化后的 `spectra_norm_ds`。卷积结构采用自适应池化，不依赖固定的最终序列长度，但本项目当前训练和验证统一以 2000 点为准。
...
- 样本数：`800000`
- 每个模型输入光谱长度：`2000`（由 20000 点原始光谱按因子 10 降采样）
...
该文件保存的是 `spectra_norm_ds`，即归一化并降采样后的模型输入；未降采样的 20000 点原始光谱不作为当前 CNN 数据文件的默认组成部分。
...
```text
work/03_ml_inverse_modeling/ML try/Residual MLP/train_spectral_cnn.py
```
...
```bat
python "work\03_ml_inverse_modeling\ML try\Residual MLP\train_spectral_cnn.py" ^
  --dataset "work\03_ml_inverse_modeling\ML try\nn_cavity_spectral_features_20260620_233057\nn_cavity_spectral_features_20260620_233057.npz" ^
  --model cnn_small ^

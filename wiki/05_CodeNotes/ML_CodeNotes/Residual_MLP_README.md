...
`train_residual_mlp_simple_pca_label_uncertainty_v2.py` 用于在不改动主数据流的前提下，给 train labels 注入人工测量误差，并比较不同误差幅度下的模型稳定性。

当前已登记运行：

- [[04_Experiments/ML_Runs/residual_mlp_simple_pca_label_uncertainty_v2_20260704_180311]]
- [[04_Experiments/ML_Runs/residual_mlp_simple_pca_label_uncertainty_v3_20260706_160929]]
- [[04_Experiments/ML_Runs/residual_mlp_simple_pca_label_uncertainty_v4_20260707_011244]]

这一路径的特点：

- 默认只跑 `uniform_pm_1nm` 场景，但把不确定度幅度扫描为 `1.0 / 1.5 / 2.0 / 2.5 / 3.0 nm`
- 输入特征固定为 nominal thickness + 部分光谱统计量 + `PC1-PC100`
- 输出重点从单一 `within +/-1 nm` 改成“随标签不确定度变化的容差带命中率”和 `excess_RMSE_after_uncertainty_nm`
...
`train_residual_mlp_simple_pca_label_uncertainty_v3.py` ?? `v2` ?????????????????? `gaussian_sigma_nm`?????? train-noise RMSE?`within_label_uncertainty_fraction` ? `excess_RMSE_after_uncertainty_nm`?????? `v2` ? `v4` ???????????????? `1.0-3.0 nm`???????? `uniform_pm` ????????????

`train_residual_mlp_simple_pca_label_uncertainty_v4.py` ? `v2` ????? train-label ????? `?10 nm` ??????? `gaussian_sigma_nm` ? `random_uniform_nm` ????????????????????????
